...
export default function Map({id, selectedMapZone}) {
    ...
    const getZoneInfo = () => {
        ...
        axiosInstance.get(`/api/zone-scores-by-datetime/?search_id=${id}&datetime=${formattedDate}`)
            .then(res => {
...
/// My handleClick
    ...
    const getZoneDetails = (zoneName) => {
        ...
        const fetchZoneData = (index = 0) => {
            ...
            axiosInstance.get(`/api/zone-details-by-search-date-zone/?search_id=${id}&date=${currentDate}&zone_id=${zoneId}`)
                .then(res => {
        ...
        return (
          <div style={{
            ...
            <div style={{
              ...
              <span style={{fontSize: '12px', color: 'white', fontWeight: 'bold'}}>Low Score</span>
              <span style={{fontSize: '12px', color: 'white', fontWeight: 'bold'}}>High Score</span>
            </div>
          </div>
        );
      };
    ...
    useEffect(() => {
        if (selectedMapZone) {
          const zone = completeZoneInfo.find(z => z.id === selectedMapZone.zone_id);
          if (zone) {
            const newSelectedZone = {
              id: zone.id,
              name: zone.name,
                    ...
                    {selectedZone && (
                        ...
                        >
                            <div className="p-2">
                                ...
                                {selectedZone.selectedHour ? (
                                    <>
                                        <p>Busyness Activity: {Math.round(selectedZone.busyness_score)}</p>
                                        <p>Time: {new Date(selectedZone.selectedHour).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                                    </>
                                ) : (
