...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for _, row in gdf.iterrows():
            ...
            zone, created = Zone.objects.get_or_create(name=name, defaults={'boundary_coordinates': boundary_coordinates})
            if created:
                self.stdout.write(self.style.SUCCESS(f'Zone "{name}" created'))
            else:
                self.stdout.write(self.style.WARNING(f'Zone "{name}" already exists'))
